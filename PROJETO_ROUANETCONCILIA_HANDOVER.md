...
### 🔴 NOVO PROBLEMA DESCOBERTO (não relacionado a documentos) — rubrica nunca foi resolvida na importação
Reportado pelo usuário: "STATUS DA REVISÃO" na aba "Lançamentos e Auditoria" continua `REVISAO_PENDENTE` pra quase tudo, mesmo com documentos vinculados hoje. Investigado: **184 de 185 transações têm `despesas.rubrica_id = NULL`**, mesmo havendo **24 rubricas cadastradas** no orçamento do projeto (`rubricas` table). O matching de rubrica (determinístico ou RAG, `motor/matching_rag.py`) falhou pra praticamente 100% dos lançamentos na importação original — isso é **anterior e independente** de tudo que foi mexido nesta sessão (storage, vinculação de documento). `status = REVISAO_PENDENTE` é setado por `rubrica_incerta` em `motor/importar.py`, não tem nada a ver com documento anexado.
**Importante**: o botão "Aprovar" de `RevisaoPendentes.tsx` (feito no início desta sessão) só força `status = PENDENTE` — NÃO atribui rubrica nenhuma. Aprovar em massa esconderia o problema (relatório de prestação de contas ficaria sem classificação orçamentária) sem resolvê-lo de verdade.
**Aguardando decisão do usuário**: investigar por que o matching de rubrica falhou e corrigir de verdade (atribuir as 24 rubricas certas aos 184 lançamentos), ou destravar o status em massa por enquanto e deixar a classificação pra depois.
