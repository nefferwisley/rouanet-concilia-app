...
5. 🟢 **LIVRE, sem dependência** — resolver a incoerência do `_disponivel()` em `revisao.py::listar_documentos_transacao` (ver nota ⚠️ na seção acima).
6. 🟢 **LIVRE, sem dependência** — decidir se a feature de "WebSocket de sincronia" (ver seção 3 mais abaixo, WIP não commitado) deve ser finalizada ou descartada — hoje o frontend tenta conectar numa rota que dá 404 no backend.
